...
public class RegistryHive : RegistryBase
{
    ...
    public byte[] ProcessTransactionLogs(List<TransactionLogFileInfo> logFileInfos, bool updateExistingData = false)
    {
        ...
        if (logOne != null && logTwo != null)
        {
            ...
            Log.Information("Two transaction logs found. Determining primary log...");
            ...
            Log.Information("Primary log: {FirstLog}, secondary log: {SecondLog}", firstLog.LogPath, secondLog.LogPath);
            ...
            {
                Log.Information("Replaying log file: {LogPath}", firstLog.LogPath);
            ...
            {
                Log.Information("Replaying log file: {LogPath}", secondLog.LogPath);
                bytes = secondLog.UpdateHiveBytes(bytes);
        ...
        if (wasUpdated)
        {
            ...
            Log.Information(
                "At least one transaction log was applied. Sequence numbers have been updated to {MaximumSequenceNumber}. New Checksum: {NewChecksum}",
                $"0x{maximumSequenceNumber:X4}", $"0x{newchecksum:X}");
        }
    ...
    private List<RegistryKey> GetSubKeysAndValues(RegistryKey key)
    {
        ...
        foreach (var valueOffset in key.NkRecord.ValueOffsets)
        {
            ...
            {
                if (valOffsetIndex < key.NkRecord.ValueListCount)
                    Log.Warning("An expected value was not found at offset {ValueOffset}. Key: {KeyPath}",
                        $"0x{valueOffset:X}", key.KeyPath);
            }
    ...
    public void ExportDataToCommonFormat(string outfile, bool deletedOnly)
    {
        ...
        using (var sw = new StreamWriter(outfile, false))
        {
            ...
            foreach (var keyValuePair in theRest)
                ...
                {
                    Log.Warning("There was an error exporting free record at offset {Offset}. Error: {Error}",
                        $"0x{keyValuePair.Value.AbsoluteOffset:X}", ex.Message);
                }
...
//                    }
        ...
        Log.Debug("Initial processing complete. Building tree...");
        ...
        Log.Debug("Found root node! Getting subkeys...");

        Root = new RegistryKey(rootNode, null);
        Log.Debug("Created root node object. Getting subkeys.");
        ...
        {
            Log.Debug("Flushing record lists...");
            ListRecords.Clear();
    ...
    {
        Log.Debug("Associating deleted keys and values...");
...
//ncrunch: no coverage
        ...
        Log.Debug("Associating top level deleted keys to active Registry keys");
        ...
        Log.Debug("Iterating unreferenced VK records");
