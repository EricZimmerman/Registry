...
namespace Registry
{
	public class RegistrySkeleton
	{
		...
		public bool Write(string outHive)
		{
			...
			var parentOffset = ProcessSkeletonTree(treeKey,true,true);//always include keys/values for now
...
;
		...
		private int ProcessSkeletonTree(SkeletonKey treeKey, bool addValues, bool addSubkeys)
		{
			var key = _hive.GetKey(treeKey.KeyPath);

			var parentOffset = ProcessKey(key, -1, addValues, addSubkeys);
