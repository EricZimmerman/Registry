...
namespace Registry.Cells
{
    ...
    internal class VKCellRecord : ICellTemplate
    {
        ...
        public VKCellRecord(byte[] rawBytes)
        {
            ...
            var dt = BitConverter.ToUInt32(rawBytes, 0x10);
            ...
            if (OffsetToData > 0)
            {
                ...
                switch (DataType)
                {
                    ...
                    case DataTypeEnum.RegMultiSz:
                        ...
                        ValueData =
                            Encoding.Unicode.GetString(datablockRaw, 4, (int)DataLength).Replace("\0", " ").Trim();
                        ValueDataSlack =
                            datablockRaw.Skip((int)(DataLength + 4))
                                .Take((int)(Math.Abs(dataBlockSize) - 4 - DataLength))
                                .ToArray();
                        // Encoding.Unicode.GetString(datablockRaw, DataLength + 4, Math.Abs(dataBlockSize) - 4 - DataLength);
